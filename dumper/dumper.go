...
func DumpDirective(d config.IDirective, style *Style) string {
	...
	if mapBlock, ok := d.(*config.Map); ok {
		return DumpMap(mapBlock, style)
	}
	if geoBlock, ok := d.(*config.Geo); ok {
		return DumpGeo(geoBlock, style)
	}
	if splitClientsBlock, ok := d.(*config.SplitClients); ok {
		return DumpSplitClients(splitClientsBlock, style)
	}
	if limitReqZone, ok := d.(*config.LimitReqZone); ok {
		return DumpLimitReqZone(limitReqZone, style)
	}
	if limitConnZone, ok := d.(*config.LimitConnZone); ok {
		return DumpLimitConnZone(limitConnZone, style)
	}
	if proxyCachePath, ok := d.(*config.ProxyCachePath); ok {
		return DumpProxyCachePath(proxyCachePath, style)
	}
	if streamBlock, ok := d.(*config.Stream); ok {
		return DumpStream(streamBlock, style)
	}
	if streamUpstream, ok := d.(*config.StreamUpstream); ok {
		return DumpStreamUpstream(streamUpstream, style)
	}
	if streamServer, ok := d.(*config.StreamServer); ok {
		return DumpStreamServer(streamServer, style)
	}
	if streamUpstreamServer, ok := d.(*config.StreamUpstreamServer); ok {
		return DumpStreamUpstreamServer(streamUpstreamServer, style)
	}
